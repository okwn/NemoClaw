...
> **Option 1: NVIDIA Endpoints:** Routes inference to models hosted on [build.nvidia.com](https://build.nvidia.com).
>
...
> **Option 2: OpenAI:** Routes inference to the OpenAI API at `https://api.openai.com/v1`.
>
...
> **Option 3: Other OpenAI-Compatible Endpoint:** Routes inference to any server that implements `/v1/chat/completions`, including OpenRouter, LocalAI, llama.cpp, vLLM behind a proxy, and any compatible gateway.
>
...
> **Option 4: Anthropic:** Routes inference to the Anthropic Messages API at `https://api.anthropic.com`.
>
...
> **Option 5: Other Anthropic-Compatible Endpoint:** Routes inference to any server that implements the Anthropic Messages API at `/v1/messages`, including Claude proxies, Bedrock-compatible gateways, and self-hosted Anthropic-compatible servers.
>
...
> **Option 6: Google Gemini:** Routes inference to Google's OpenAI-compatible Gemini endpoint at `https://generativelanguage.googleapis.com/v1beta/openai/`.
>
...
> **Option 7: Local Ollama:** Routes inference to a local Ollama instance. Depending on your platform, the wizard can use an existing daemon, start an installed daemon, or offer an install action.
>
...
> **Option 8: Model Router:** Starts a host-side model router and routes sandbox inference through OpenShell to that router.
...
> **Experimental: Local NIM and Local vLLM:** These options appear when `NEMOCLAW_EXPERIMENTAL=1` is set and the prerequisites are met.
>
