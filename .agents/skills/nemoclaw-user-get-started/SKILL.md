...
description: "Installs NemoClaw, selects the Hermes agent, and launches a sandboxed Hermes API endpoint. Use when users ask for Hermes setup, NemoHermes onboarding, or running Hermes inside OpenShell. Trigger keywords - nemohermes quickstart, hermes agent nemoclaw, run hermes openshell sandbox, nemoclaw quickstart, install nemoclaw openclaw sandbox, nemoclaw prerequisites, nemoclaw supported platforms, nemoclaw hardware software, nemoclaw windows wsl2 setup, nemoclaw install windows docker desktop."
---
...
## Step 3: Use Non-Interactive Setup
...
- **Load [references/quickstart.md](references/quickstart.md)** when onboarding, installing, or launching a NemoClaw sandbox for the first time. Installs NemoClaw, launches a sandbox, and runs the first agent prompt.
...
- `nemoclaw-user-configure-inference` — Inference Options (use the `nemoclaw-user-configure-inference` skill) to choose a provider and model
- `nemoclaw-user-reference` — Commands (use the `nemoclaw-user-reference` skill) to see the full `nemohermes` alias behavior
- `nemoclaw-user-workspace` — Backup and Restore (use the `nemoclaw-user-workspace` skill) to preserve sandbox state before destructive operations
- `nemoclaw-user-monitor-sandbox` — Monitor Sandbox Activity (use the `nemoclaw-user-monitor-sandbox` skill) to inspect OpenShell events and sandbox logs
