...
printf '%s' "$fingerprint" | jq -r '.symbols[]' | while read -r sym; do
  [ -z "$sym" ] && continue
  search_term "\"$sym\" in:body,comments,title" "$PER_SYMBOL_TOP" >>"$candidates_file"
done

# Per-file-path search.
printf '%s' "$fingerprint" | jq -r '.files[]' | while read -r path; do
  [ -z "$path" ] && continue
  search_term "\"$path\" in:body,comments" "$PER_FILE_TOP" >>"$candidates_file"
done
...
printf '%s' "$fingerprint" | jq -r '.error_strings[]' | while read -r str; do
  [ -z "$str" ] && continue
  search_term "\"$str\" in:body,comments" "$PER_ERROR_TOP" >>"$candidates_file"
done

# Dedupe, exclude primary issue, cap total.
candidates=$(sort -u "$candidates_file" \
  | grep -v "^${primary_issue}$" \
  | head -n "$MAX_TOTAL")
