...
If the sandbox is not reachable (stopped, SSH outage) when the cleanup runs for a QR-paired channel, the command refuses to proceed to the rebuild and asks you to start the sandbox and re-run, so a half-removed state cannot leave stale Baileys auth files behind for the next rebuild to restore.
