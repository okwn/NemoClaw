...
| Route inference | Configures OpenShell inference routing so agent traffic goes to the provider and model you chose during onboarding. The agent uses `inference.local` inside the sandbox; credentials stay on the host. |
...
Without guardrails, this creates security, cost, and compliance risks that grow as agents run unattended.
...
| Routed inference | Model traffic is routed through the OpenShell gateway to your selected provider, transparent to the agent. You can switch providers or models. |
...
You can use NemoClaw for the following use cases.
...
- Read [How It Works](/about/how-it-works) to understand how NemoClaw runs.
- Read [Ecosystem](/about/ecosystem) to understand how OpenClaw, OpenShell, and NemoClaw relate in the wider stack.
